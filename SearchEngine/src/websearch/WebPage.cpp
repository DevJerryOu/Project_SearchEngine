#include "WebPage.hpp"
WebPage::WebPage(std::string &str, Configuration &conf, WordSegmentation &wordsegmentation)
{ //构造函数里调用文档的处理函数
...
std::string WebPage::getContent() const
{
    return _docContent;
}
std::string WebPage::getDoc() const
{
    return _doc;
}
std::string WebPage::getSummary() const
{
    return _docSummary;
}
map<std::string, int> &WebPage::getWordsMap()
{
    return _wordsMap;
}
void WebPage::processDoc(const std::string &str, Configuration &conf, WordSegmentation &wordsegmentation)
{
    //先提取网页库的std::string 提取出每个std::string的id，title，url，content，summary
    ...
    std::string cutWord = _docTitle + _docContent;
    vector<std::string> vec = wordsegmentation.cut(cutWord);
...
void WebPage::calcTopK(vector<std::string> &wordsVec, int k, set<std::string> &stopWordList)
{
    set<std::string> st2;
